...
def get_image(client,VehicleName):
    ...
    responses = client.simGetImages([airsim.ImageRequest("CameraOF",8,pixels_as_float=True)], 
    vehicle_name=VehicleName)
    imgWide = responses[0]
    w,h=imgWide.width,imgWide.height
    imgWide = airsim.list_to_2d_float_array(imgWide.image_data_float, imgWide.width, imgWide.height)


    
    '''try:
        imgWide = airsim.list_to_2d_float_array(response.image_data_float, response.width, response.height)
    ...
    return torch.reshape(torch.tensor(imgWide),shape=(1,1,128,128)),w,h
